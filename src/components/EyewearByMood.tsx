...
const EyewearByMood: React.FC = () => {
  ...
  return (
    <section  id="eyewear-by-mood" className="px-6 sm:px-12 md:px-20 py-20 bg-gradient-to-br from-teal-50 to-green-50">
      <div className="container mx-auto px-0">
        ...
        <div className="sm:hidden mb-6 flex justify-center">
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-full font-semibold shadow-md hover:bg-teal-700 transition-colors duration-300"
            aria-label="Toggle filters"
          >
            <Filter className="w-5 h-5" />
            Filters
            {showFiltersMobile && <X className="w-5 h-5" />}
          </button>
        </div>
          ...
          <div className="sm:hidden flex flex-wrap justify-center gap-3 mb-12 px-4">
            {availableFilters.map(filter => (
              <button
                ...
                className={`px-5 py-2 rounded-full font-semibold transition-all duration-300 transform hover:scale-105 whitespace-nowrap ${
                  activeFilter === filter.id
                    ? `${filter.color} text-white shadow-lg scale-105`
                    : 'bg-white text-gray-700 border-2 border-gray-200 hover:border-gray-300'
                }`}
        ...
        {/* Products Flexbox - make container max width smaller on small screens */}
        ...
        <div className="text-center mt-8">
          <a
            ...
            className="inline-block px-8 py-3 bg-indigo-600 text-white font-semibold rounded-full hover:bg-indigo-700 transition-colors duration-300 transform hover:scale-105 shadow-lg"
          >
            See More on Amazon
          </a>
        </div>
